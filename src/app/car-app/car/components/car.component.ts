...
import { carFeatureKey } from '../reducers';
import { availableOptionsFeatureKey } from '../available-options/available-options.reducer';
...
export class CarComponent implements OnInit {
  ...
  ngOnInit() {
    this.state$ = this.store.pipe(select(carFeatureKey));
    ...
    this.store
      .pipe(select(carFeatureKey, availableOptionsFeatureKey))
      .subscribe((options: string[]) => {
        const formGroup = this.form.get('options') as FormGroup;

        this.availableOptions.forEach(option => {
          if (!options.includes(option)) {
            formGroup.removeControl(option);
          }
        });

        options.forEach(option => {
          if (!this.availableOptions.includes(option)) {
